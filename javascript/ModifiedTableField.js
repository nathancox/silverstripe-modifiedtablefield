...
TableField.prototype = {
	...
	initialize: function() {
		...
		// added this block to clumsily and hackily associate the defaults variable with the table element
		// so we can get it easily in addRow
		var fieldName = this.id.split('_')[2];
		var defaults = {};
		if (typeof window[fieldName+'_fieldDefaults'] ==  'object') {
			defaults = window[fieldName+'_fieldDefaults'];
		}
		...
		for(var i = 0; i < inputs.length; i++) {
			var idBits = inputs[i].id.split('-');
			var fieldName = idBits[idBits.length-1];
			
			if (defaults.hasOwnProperty(fieldName)) {
				inputs[i].setAttribute('data-defaultvalue', defaults[fieldName]);
			}
		}
		
		if (table.tBodies[0].rows.length == 1) {
			this.setFieldsToDefault(inputs);
		}
		...
		/*
			...
			if(inputs[i].type == 'checkbox') {
				if (defaults.hasOwnProperty(fieldName) && defaults[fieldName] == 1) {
	...
	deleteRecord: function(e) {
		...
		if(
			...
			if( row.parentNode.getElementsByTagName('tr').length > 1 ) {
				...
				if( fields )
					this.setFieldsToDefault(fields);
			}
			Event.stop(e);
			return false;
		}
	...
	setFieldsToDefault: function(inputs) {
		
			// For every input, set it's value to blank if it is not hidden
			for(var i = 0; i < inputs.length; i++) {
				
				var defaultValue = inputs[i].getAttribute('data-defaultvalue');
				if(inputs[i].type == 'checkbox') {
					if (defaultValue == 1) {
						inputs[i].checked = "checked";
					} else {
					...
					if (defaultValue) {
						...
						inputs[i].setAttribute('value',  defaultValue);
					} else {
						inputs[i].value = '';
